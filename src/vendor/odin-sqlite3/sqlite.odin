...
@(private)
USE_CUSTOM_FTS5 :: #config(SQLITE3_CUSTOM_FTS5, false)
...
when ODIN_OS == .Windows {
	...
	when USE_CUSTOM_FTS5 {
		foreign import sqlite "libsqlite3-fts5.dylib"
	} else when USE_SYSTEM_LIB {
		when USE_DYNAMIC_LIB {
			when USE_SQLCIPHER {
	...
	when USE_CUSTOM_FTS5 {
		foreign import sqlite "libsqlite3-fts5.so"
	} else when USE_SYSTEM_LIB {
		when USE_DYNAMIC_LIB {
			when USE_SQLCIPHER {
